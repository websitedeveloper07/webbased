...
if (isset($_GET['telegram_auth']) && !empty($_GET['id']) && !empty($_GET['hash'])) {
    error_log("Telegram callback received: " . json_encode($_GET));
    $telegramData = [
        'id' => $_GET['id'] ?? '',
    ...
    if (verifyTelegramData($telegramData, $telegramBotToken)) {
        ...
        if (checkTelegramAccess($telegramId, $telegramBotToken)) {
            ...
            if (isset($_SESSION['user'])) {
                error_log("Session successfully set for user: telegram_id=$telegramId | Session ID: " . session_id());
            } else {
            ...
            echo '<script>if (window.location.href === "' . $baseUrl . '/login.php") { window.location.href = "' . $baseUrl . '/index.php"; }</script>';
            ob_end_flush();
            exit;
        } else {
