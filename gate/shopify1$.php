...
if (!isset($_SESSION['admin_authenticated']) || $_SESSION['admin_authenticated'] !== true) {
    if (!isset($_SESSION['user']) || $_SESSION['user']['auth_provider'] !== 'telegram') {
        error_log("Unauthorized access to shopify1$.php: Invalid session");
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['card']) || !is_array($_POST['card'])) {
    ...
    echo json_encode([
        'status' => 'ERROR',
        'message' => 'Invalid request or missing card data'
    ]);
    exit;
}
...
if (!preg_match('/^(0[1-9]|1[0-2])$/', $exp_month)) {
    ...
    echo json_encode([
        'status' => 'ERROR',
        'message' => 'Invalid exp_month format'
    ]);
    exit;
}
...
if (strlen($exp_year_raw) == 2) {
    ...
    echo json_encode([
        'status' => 'ERROR',
        'message' => 'Invalid exp_year format - must be YY or YYYY'
    ]);
    exit;
}
...
if (!preg_match('/^\d{4}$/', (string) $exp_year) || $exp_year > (int) date('Y') + 10) {
    ...
    echo json_encode([
        'status' => 'ERROR',
        'message' => 'Invalid exp_year format or too far in future'
    ]);
    exit;
}
...
// Check single card and return JSON response
