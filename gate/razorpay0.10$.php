...
// Add rate limiting check
 $rate_limit_key = 'razorpay_rate_limit_' . ($_SESSION['user']['id'] ?? 'unknown');
...
// Optional file-based logging for debugging (disable in production)
 $log_file = __DIR__ . '/razorpay0.10$_debug.log';
function log_message($message) {
    global $log_file;
    file_put_contents($log_file, date('Y-m-d H:i:s') . " - $message\n", FILE_APPEND);
}

// Function to check a single card via Razorpay API with retry
function checkCard($card_number, $exp_month, $exp_year, $cvc, $retry = 1) {
    ...
    for ($attempt = 0; $attempt <= $retry; $attempt++) {
        ...
        $status = 'DECLINED';
        if ($response_text === 'Payment successful') {
            $status = 'CHARGED';
        } elseif (in_array($response_text, [
            'Payment declined',
            'Your payment has been cancelled. Try again or complete the payment later.',
            '3dsecure is not enabled for the card by the cardholder or the bank/issuer'
        ])) {
            $status = 'DECLINED';
        } else {
            // Assume any other response is a success (APPROVED)
            $status = 'APPROVED';
        }
